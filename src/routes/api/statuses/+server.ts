...
export const GET: RequestHandler = async () => {
  try {
    ...
    const response = await fetch(`${BASE_URL}/v1/incident_statuses`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return json(
        { error: 'Failed to fetch statuses', status: response.status },
    ...
    return json(data, {
      headers: {
        'Cache-Control': 'max-age=3600', // Cache for 1 hour (statuses don't change often)
      },
    });
  } catch (error) {
